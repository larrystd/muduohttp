...
void RpcServer::registerService(google::protobuf::Service* service) // 向rpcserver中注册service google::protobuf::Service
{
  const google::protobuf::ServiceDescriptor* desc = service->GetDescriptor();
  services_[desc->full_name()] = service; // name->google::protobuf::Service
}
...
void RpcServer::onConnection(const TcpConnectionPtr& conn)
{
  ...
  if (conn->connected())
  {
    RpcChannelPtr channel(new RpcChannel(conn));  // 创建一个RpcChannelPtr channel
    channel->setServices(&services_); // channel的service_
    // 设置message回调,&RpcChannel::onMessage
    conn->setMessageCallback(
        std::bind(&RpcChannel::onMessage, get_pointer(channel), _1, _2, _3));
    conn->setContext(channel);  // context
  }
  else
  {
