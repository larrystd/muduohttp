...
class ProtobufCodecLite : noncopyable // 编解码的实现函数
{
 public:
  ...
  ProtobufCodecLite(const ::google::protobuf::Message* prototype,
    ...
    : prototype_(prototype),
      ...
      messageCallback_(messageCb),  // 回调,message处理
      rawCb_(rawCb),
 ...
 private:
  ...
  const string tag_;
  ProtobufMessageCallback messageCallback_; // parse 反序列化之后, 调用message回调函数执行message处理
...
class ProtobufCodecLiteT  // 作用就是编解码, 调用class ProtobufCodecLite
{
 ...
 public:
  ...
  explicit ProtobufCodecLiteT(const ProtobufMessageCallback& messageCb,
    ...
    : messageCallback_(messageCb),  // message callback
