...
#include "protorpc/protobuf_codec/ProtobufCodecLite.h"
...
void ProtobufCodecLite::fillEmptyBuffer(muduo::net::Buffer* buf,
                                        const google::protobuf::Message& message) // 将google::protobuf::Message& message填充buf
{
  assert(buf->readableBytes() == 0);
  // FIXME: can we move serialization & checksum to other thread?
  buf->append(tag_);

  int byte_size = serializeToBuffer(message, buf);  // 将message序列化到buf,一步解决

  int32_t checkSum = checksum(buf->peek(), static_cast<int>(buf->readableBytes())); // 加入校验和,hostToNetwork32 
...
void ProtobufCodecLite::onMessage(const TcpConnectionPtr& conn,
                                  Buffer* buf,
                                  Timestamp receiveTime)  // 接受传来的信息, 首先就是parse序列到message class中, 然后调用messageCallback_
{
  while (buf->readableBytes() >= static_cast<uint32_t>(kMinMessageLen+kHeaderLen))
  {
    ...
    else if (buf->readableBytes() >= implicit_cast<size_t>(kHeaderLen+len))
    {
      ...
      MessagePtr message(prototype_->New());  // 得到一个Messageptr， std::shared_ptr<google::protobuf::Message>
      // FIXME: can we move deserialization & callback to other thread?
      ErrorCode errorCode = parse(buf->peek()+kHeaderLen, len, message.get());  // 将buf解析到message之中
      if (errorCode == kNoError)
      {
        // FIXME: try { } catch (...) { }
        messageCallback_(conn, message, receiveTime); // 基于message执行messageCallback_
        buf->retrieve(kHeaderLen+len);
      }
      else
      {
...
int ProtobufCodecLite::serializeToBuffer(const google::protobuf::Message& message, Buffer* buf) // 将对象, google::protobuf::Message&序列化为字符串加入到Buffer中
{
  ...
  uint8_t* start = reinterpret_cast<uint8_t*>(buf->beginWrite()); //message 序列化到buffer中
  uint8_t* end = message.SerializeWithCachedSizesToArray(start);  // 核心调用SerializeWithCachedSizesToArray序列化
  ...
  buf->hasWritten(byte_size); // buf写完，更新writerIndex_
  return byte_size;
}
